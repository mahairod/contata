...
доступный класс Вариант расширяет ПодОбъектСинт<Фрагмент> {
	...
	доступный Список<Группа> дайГруппы() {
		верни группы;
	}

	доступный Список<Юнит> дайЮниты() {
		верни юниты;
	}
	...
	защищённый тщетный постОбработка(){
		...
		для (Юнит юнит : юниты) {
			Слово слово = слова.дай(индекс);
			юнит.омоним = слово.getОмонимы().дай( юнит.getНомер() );
			юнит.омоним.юнит = юнит;
			юнит.омоним.слово = слово;
			индекс++;
		}
	}
