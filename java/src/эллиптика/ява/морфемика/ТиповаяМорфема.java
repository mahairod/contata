...
доступный абстрактный класс ТиповаяМорфема расширяет Объект воплощает Морфема {
	...
	доступный логическое применимК(Морфема то) {
		верни это.базоваяЧастьРечи() == то.целеваяЧастьРечи() && это.ставитсяВозле().содержит(то.тип());
	}
	...
	защищённый Множество<ТипМорфемы> ставитсяВозле() {
		верни тип() == ТипМорфемы.ПРИСТАВКА ? ставитсяПеред() : ставитсяПосле();
	}
	...
	доступный Строка строкой() {
		Строка код = значение;
		если (тип().ordinal() < ТипМорфемы.ДИСФИКС.ordinal()) {
			код = код + "-";
		}
		если (тип().ordinal() > ТипМорфемы.КОРЕНЬ.ordinal()) {
			код = "-" + код;
		}
		верни код + "(" + тип().name().вНижнемРегистре() + "," + базоваяЧастьРечи().кратко() + "->" + целеваяЧастьРечи().кратко() + ")";
	}

	@Подмени
	доступный абстрактный ТипМорфемы тип();
