...
доступный класс Подборщик {
	...
	личный Список<Список<Морфема>> подбери_(Строка искомое, ЧастьРечи частьРечи, Строка основа){
		...
		Список<Вариант> вар = поиск(основа, частьРечи, ничто, ок.пуста() ? '∅': ок.символВ(0), ничто);
	...
	тщетный обходМорфем(Вариант вариант, Список<Морфема> списокМорфем, Список<Список<Морфема>> списки) {
		...
		если (вариант.getДети() == ничто){
			...
			Система.вывод.print(вариант.дайОстаток());
			для (цел и = списокМорфем.размер()-1; и >=0; и--){
				Система.вывод.print(списокМорфем.дай(и).значение());
			}
	...
	личный Список<Вариант> поиск(итоговый Строка искомое, итоговый ЧастьРечи частьРечи, Множество<ЧастьРечи> исключения, символ последующий, Строка язык){
		...
		для (ЗаписьМорфемы морфема: варианты){
			...
			символ начало = длинаСуф==0 ? 'ь' : морфема.getОбрат().символВ(длинаСуф-1);
			Список<Вариант> рез = поиск(искомое_, текЧастьРечи, ИСКЛЮЧЁННЫЕ_СВЯЗКИ.дай(морфема.целеваяЧастьРечи()), начало, язык);
			если (!рез.пусто()){
				вывод("-успех!");
