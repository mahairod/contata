...
доступный переч Граммема {
	...
	Граммема(ТипГраммемы типГраммемы) {
		...
		типГраммемы.обновиГраницы(это);
	}

	личный итоговый ТипГраммемы типГраммемы;

	доступный Строка коротко() {
		если (ordinal() <= неодушевлённое.ordinal() ) {
	...
	доступный статичный ширцел маскаГраммем(Коллекция<Граммема> граммемы) {
		ширцел результат = 0;
	...
	доступный статичный Множество<Граммема> дайГраммемы(ширцел маскаГраммем) {
		Множество<Граммема> результат = новый МножествоСвёрток<>(Long.bitCount(маскаГраммем));
		Граммема[] граммемы = Граммема.values();
		пока (маскаГраммем!=0) {
			ширцел бит = Long.lowestOneBit(маскаГраммем);
			маскаГраммем ^= бит;
