...
доступный класс Лингвистика {
	...
	доступный статичный Object подбериМножЧислоФразы(символ[] символыФразы) {
		верни делегЛинг().подбериМножЧислоФразы(с(символыФразы));
	}

	доступный статичный Object подбериЗаданнуюФормуФразы(символ[] символыФразы, Граммема ... целевыеГраммемы) {
		верни делегЛинг().подбериЗаданнуюФормуФразы(с(символыФразы), Массивы.списком(целевыеГраммемы));
	}

	доступный статичный Object подбериНачальнуюФормуФразы(символ[] символыФразы, Граммема ... исходныеГраммемы) {
		верни делегЛинг().подбериНачальнуюФормуФразы(с(символыФразы), Массивы.списком(исходныеГраммемы));
	}

	доступный статичный Object измениФормуСлова(символ[] символыФразы, ЧастьРечи частьРечи, Граммема ... целевыеГраммемы) {
		верни делегЛинг().измениНаЗаданнуюФормуСлова(с(символыФразы), частьРечи, Массивы.списком(целевыеГраммемы));
	}

	доступный статичный Object согласуйФормуСлова(символ[] образец, символ[] исходное, ЧастьРечи частьРечи) {
		верни делегЛинг().согласуйФормуСлова(с(образец), с(исходное), частьРечи);
	}
	...
	доступный статичный Object сложноеСлово(символ[] начало, символ[] конец, Сложение.Характер характер, Сложение.Слитность слитность) {
		верни делегЛинг().сложноеСлово(с(начало), с(конец), характер, слитность);
	}

	доступный статичный Граммема[] определиФормуФразы(символ[] символыФразы) {
		Коллекция<Граммема> граммемы = делегЛинг().определиФормуФразы(с(символыФразы));
		если (граммемы != ничто){
	...
	личный статичный Строка с(символ[] символы){
		верни новый Строка(символы);
	}

	доступный статичный Граммема[] определиРодФразы(символ[] символыФразы) {
		верни определиГраммемыФразыДанногоТипа(символыФразы, ТипГраммемы.Род);
	}

	доступный статичный Граммема[] определиГраммемыФразыДанногоТипа(символ[] символыФразы, ТипГраммемы типГраммемы) {
	...
	личный статичный ДелегатЛингвистики делегЛинг() {
