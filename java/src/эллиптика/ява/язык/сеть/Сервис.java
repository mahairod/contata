...
доступный абстрактный класс Сервис расширяет ОбработчикППСТ воплощает Сервисок {
	личный статичный итоговый Logger ЖУРНАЛ = Logger.getLogger(Сервис.класс.дайИмя());
	...
	защищённый тщетный займись(РазменППСТ размен) кидает IOException {
		...
		StringBuilder пс = новый StringBuilder();
		цел ответ;
		попробуй {
			ответ = сервисок.обработай(размен, пс);
		} ловя (Exception откл) {
			ЖУРНАЛ.log(Level.SEVERE, ничто, откл);
			ответ = HttpURLConnection.HTTP_INTERNAL_ERROR;
			пс.setLength(0);
			пс.trimToSize();
			пс.append("Произошла ошибка");
		}
	...
	личный статичный класс ОшибочныйСервисок воплощает Сервисок {
		...
		доступный цел обработай(Строка запрос, StringBuilder вывод) кидает IOException {
			вывод.append("Путь не обслуживается");
