...
void CMAPost::log(string s){
    const char* logFName = m_LogFileName.c_str();
    if (! *logFName)  return;
	try {
...
void CMAPost::RunRules()
{
	try
	{
		...
		ErrorMessage("Mapost", "Exception caught in Mapost");
		throw;
	} catch(...) {
...
void CMAPost::Rule_Interjections()    
{
    ...
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
        if (!it->m_bSpace)
        {
            ...
            if (W.HasDes(ORLE)) 
            {
                if (W.m_strUpperWord == RCNST_A || W.m_strUpperWord == RCNST_NO)
                {
...
string CMAPost::GetSimilarNumAncode (const string&  Lemma, const string&  Flexia, bool IsNoun) 
{
    ...
    if ( (POS == NUMERAL) ||  (POS == NUMERAL_P) || Lemma == RCNST_NULEVOY)
	    break;
	};
  	...
  	for (k=0; k < P.GetCount(); k++)
	{
		  ...
		  if ( IsNoun && Form != h && string(RCNST_ANCODES).find(P.GetAncode(k)) != string::npos ) // 1000 - не аббр, "свыше 1000 человек"
			  continue;
...
void CMAPost::Cifrdef()    
{
	...
	for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
		...
		if( !strcmp(NumeralToNumber[i].m_Cardinal, RCNST_ODIN)) AnCodes = RCNST_GENANCS; //все грамкоды с родом
        ...
        else
        {
			...
			if  ( !(AnCodes0.empty() || ( next_it !=  m_Words.end() && (next_it->m_strUpperWord == RCNST_GG) ) ) ) // ЧИСЛ
			{
				CHomonym* pH = W.AddNewHomonym();
				pH->SetMorphUnknown();
			...
			if( next_it !=  m_Words.end() && (next_it->m_strWord == "%" || next_it->m_strWord == "$"))  //доллары, проценты
				spec_it = next_it;
			else if ( it !=  m_Words.begin() && (prev_it->m_strWord == "$"||prev_it->m_strWord == RCNST_NUM))
				spec_it = prev_it;
			if( it != spec_it)
			{
				...
				pH->SetMorphUnknown();
				pH->m_CommonGramCode = RCNST_FA;	
				pH->SetGramCodes(RCNST_AO);
				if ( W2.m_strWord == "%" )
				{
					W2.m_strUpperWord = W2.m_strWord = RCNST_PROC;
					pH->m_strLemma = RCNST_PROCENT;
				}
				else  if ( W2.m_strWord == "$" )
				{
					if(spec_it == prev_it) //$12
						dollar = prev_it;
					W2.m_strUpperWord = W2.m_strWord = RCNST_DOLL;
					pH->m_strLemma = RCNST_DOLLAR;
				}
				else  if ( W2.m_strWord == RCNST_NUM )
				{
					W2.m_strUpperWord = W2.m_strWord = RCNST_NUM;
					pH->m_strLemma = RCNST_NOMER;
					//pH->m_GramCodes = "";
				}
				...
				if  ( !(( next_it !=  m_Words.end() && (next_it->m_strUpperWord == RCNST_LET))	// "в течение 2 лет"
...
void CMAPost::PronounP_Pronoun_Homonymy() 
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        ...
        if (   (W.m_strUpperWord == RCNST_ETO) 
            || (W.m_strUpperWord == RCNST_TO) 
            || (W.m_strUpperWord == RCNST_IH) 
            || (W.m_strUpperWord == RCNST_EGO) 
            || (W.m_strUpperWord == RCNST_EE) 
            || (W.m_strUpperWord == RCNST_EYO) 
            )
            continue;
...
void CMAPost::OtherRules() 
{
	for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
		...
		if( W.HasDes(ONumChar) && is_alpha((BYTE)W.m_strWord[0]) && W.GetPoses()==0 )
		{
            ...
            pNew->SetMorphUnknown();
            pNew->m_strLemma = W.m_strUpperWord;
			pNew->m_CommonGramCode = RCNST_FA;
...
void CMAPost::SemiAdjectives() 
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        ...
        if ( W.m_strUpperWord.substr(0,4) != RCNST_POLU) continue;
        for (int i=0; i < W.GetHomonymsCount(); i++)
            W.GetHomonym(i)->m_strLemma.erase(0,4);
        // установка графематической пометы
        W.m_UnparsedGraphemDescriptorsStr  += " #" RCNST_POLU " ";
    };
};
...
void CMAPost::SemiNouns() 
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        ...
        {
            if (W.m_strUpperWord.substr(0,4) == RCNST_POLU)
                PrefixLen = 4;
            else
               if ( W.m_strUpperWord.substr(0,4) == RCNST_POL "-")
               {
                   PrefixLen = 4;
                   bChangeToPlural = true;
               }
               else
                if (W.m_strUpperWord.substr(0,3) == RCNST_POL)
                {
                    bChangeToPlural = true;
                    PrefixLen = 3;
                }
...
void CMAPost::Rule_UZHE() 
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        CPostLemWord& W = *it;
        if ( !W.HasDes(ORLE) ) continue;
        if (W.m_strUpperWord !=  RCNST_UJE) continue;
...
void CMAPost::Rule_Ideclinable() 
{
	for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
		...
		if ((BYTE)WordForm[WordForm.length() - 1] != (BYTE)RCNST_O) continue;
        for (int i=0; i < W.GetHomonymsCount(); i++)
        {
            CHomonym* pH = W.GetHomonym(i);
...
void CMAPost::Rule_ILE()    
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
        ...
        if ( W.HasDes(OLLE) ) 
		{
			...
			pNew->m_CommonGramCode = RCNST_FA;
			//if ( W.HasDes(OUp) ) 
			//	pNew->m_CommonGramCode += RCNST_AOATATS;//m_DURNOVOGramCode;
			pNew->SetGramCodes ( m_pRusGramTab->GetGramCodes(NOUN, rAllCases | rAllNumbers | rAllGenders, 0));//m_DURNOVOGramCode;//"Йшааабавагадаеажазаиайакаласгагбгвгггдгегжгзгигйгкглеаебевегедееежезеиейекелижизииийикил";
            pNew->InitAncodePattern();
		}
	};
...
void CMAPost::Rule_KAK_MOZHNO()    
{
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        ...
        CPostLemWord& W = *it;
        if (!W.HasDes(ORLE)) continue;
        if (W.m_strUpperWord != RCNST_KAK) continue;
        if (W.HasOborot1() != W.HasOborot2()) continue;// "т.е." - это оборот, его обрабатывать не надо

        // ищем словоформу "можно"
        CLineIter mozno_it = NextNotSpace(it);
        if ( mozno_it == m_Words.end() ) break;
        if ( mozno_it->m_strUpperWord != RCNST_MOJNO) continue;
        ...
        if (compar_it->m_strUpperWord == RCNST_BOLEE || compar_it->m_strUpperWord == RCNST_MENEE)
        ...
        compar_it->m_UnparsedGraphemDescriptorsStr += " #" RCNST_KAK "_" RCNST_MOJNO " ";
        m_Words.erase(it);
...
void CMAPost::Rule_Redublication()    
{
	for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
        ...
        while (true)
        {
            ...
            if (duble_word->m_strUpperWord == RCNST_NE)
            {
        ...
        if (last_it != it)
        {
            ...
            W.m_UnparsedGraphemDescriptorsStr += RCNST_REDUPL;
            if (bHasEndOfSent)
                W.AddDes(OSentEnd);
        }
    }
...
void CMAPost::Rule_VOT_UZHE() 
{
    ...
    for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
    {
        CPostLemWord& W = *it;
        if (!W.HasDes(ORLE)) continue;
        if (W.m_strUpperWord != RCNST_VOT) continue;

        CLineIter next_it = NextNotSpace(it);
        if ( next_it == m_Words.end() ) return;  
        if ( next_it->m_strUpperWord != RCNST_UJE ) continue;
        ...
        {
            if (next_it->m_strUpperWord == RCNST_KAK) 
            {	
                bFound = true;
                break;
            };
        ...
        P.AddDes(ORLE);
        P.AddDes(OLw);
        P.LemmatizeForm(RCNST_PROSHLI, m_pRusLemmatizer);
        P.SetWordStr(RCNST_PROSHLI, morphRussian);
...
{	
   	for (CLineIter it=m_Words.begin(); it !=  m_Words.end(); it++)
	{
        ...
        if (W.m_strUpperWord.substr(0,3) != RCNST_PO_) continue;
...
void CMAPost::Rule_TwoPredicates()
{
	...
	{
	    CPostLemWord& W = *it;
        if (W.HasDes(OSentEnd) || W.HasDes(OPun) || W.m_strUpperWord == RCNST_I || W.m_strUpperWord == RCNST_ILI)
        {
            CLineIter end_it = it;
            if (W.HasDes(OSentEnd))
                end_it++;
