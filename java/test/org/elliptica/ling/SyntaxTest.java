...
доступный класс SyntaxTest {
	...
	доступный тщетный testРазборТекста() {
		Система.вывод.печатайстр("РазборТекста");
...
//	Строка разделитель = "(?=[а-яё]).{0}(?=[А-ЯЁ])";
	...
	доступный тщетный проверкаРазбораФразы() {
		Система.вывод.печатайстр("РазборФразы");
		Строка вход = "главнаяЧастьРусскогоМетроВМоскве";
...
*/
		...
		Строка вывод = предст.поток().reduce((стр1, стр2) -> стр1 + "\n" + стр2 ).orElse("Нет вывода") ;
//		assertTrue(вывод, ложь);
		
		Коллекция<Граммема> граммемы = гСтр.дайГраммемы();
		граммемы.удали(Граммема.единственное);
		граммемы.добавь(Граммема.множественное);
		гСтр.смениФорму(граммемы);

		предст = гСтр.представление();
		вывод = предст.поток().reduce((стр1, стр2) -> стр1 + "\n" + стр2 ).orElse("Нет вывода") + "\n" ;
		assertTrue(вывод, ложь);
...
//	@Test
	доступный тщетный testFinalize() кидает Throwable {
		Система.вывод.печатайстр("finalize");
		instance.finalize();
	}
