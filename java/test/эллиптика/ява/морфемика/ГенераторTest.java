...
доступный класс ГенераторTest {
    ...
    доступный статичный тщетный setUpClass() {
		instance = новый Генератор();
    }
	статичный Генератор instance;
	...
	тщетный запускСущ_(Строка expResult, Граммема... граммемы){
		...
		Строка result = instance.запускСБазой(метод, ЧастьРечи.существительное, грамм);
		Строка рез = итог();
		assertEquals(expResult, рез);
	}

	личный Строка итог(){
		верни instance.итог.поток().map(спис -> спис.поток().map(морф -> морф.дайМорфему()).reduce("", (л,п) -> л+п)).reduce("", (л,п) -> л+"/"+п);
	}
