...
доступный класс MorphTest {
	...
	доступный тщетный testПриготовьСловари() {
		Система.вывод.печатайстр("ПриготовьСловари");
	...
	доступный тщетный testЗакройСловари() {
		Система.вывод.печатайстр("ЗакройСловари");
		попробуй{
			Morph.закройСловари();
		} ловя (ОтклонениеМорфологии ом){
			fail("Метод вызвал ошибку " + ом.getMessage());
	...
	доступный тщетный testНайдиСлово() {
		Система.вывод.печатайстр("НайдиСлово");
		Строка слово = "проверка";
		Строка ожидаемое = слово.вВерхнемРегистре();
		РезультатСлова результат = Morph.найдиСлово(слово);
		assertEquals(ожидаемое, результат.дайПарадигмы().итератор().следующий().дайБазовуюФорму());
	}
	...
	доступный тщетный testНайдиФорму() {
		Система.вывод.печатайстр("НайдиФорму");
		Строка слово = "";
		Множество<Граммема> граммемы = ничто;
		ФормаСлова ожидаемое = ничто;
		ФормаСлова результат = Morph.найдиФорму(слово, ЧастьРечи.существительное, граммемы);
	...
	доступный тщетный testФормыСлова() {
		Система.вывод.печатайстр("ФормыСлова");
		Строка слово = "";
