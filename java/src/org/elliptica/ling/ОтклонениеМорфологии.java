/*
 * Авторское право принадлежит Антону Александровичу Астафьеву <anton@astafiev.me> (Anton Astafiev) ѱ.
 * Все права защищены и охраняются законом.
 * Copyright (c) 2016 Антон Александрович Астафьев <anton@astafiev.me> (Anton Astafiev). All rights reserved.
 * 
 *  Собственная лицензия Астафьева
 * Данный программный код является собственностью Астафьева Антона Александровича
 * и может быть использован только с его личного разрешения
 */

пакет org.elliptica.ling;

/**
 *
 * @author Антон Александрович Астафьев {@literal <anton@astafiev.me>}
 */
доступный класс ОтклонениеМорфологии расширяет RuntimeException {

	личный статичный итоговый ширцел serialVersionUID = 6844719078250020184L;

	доступный ОтклонениеМорфологии() {
		поверх();
	}

	доступный ОтклонениеМорфологии(Строка сообщение, Throwable причина) {
		поверх(сообщение, причина);
	}

	доступный ОтклонениеМорфологии(Строка сообщение) {
		поверх(сообщение);
	}

	доступный ОтклонениеМорфологии(Throwable причина) {
		поверх(причина);
	}

}
