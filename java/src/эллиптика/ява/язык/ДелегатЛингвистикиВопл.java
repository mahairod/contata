...
класс ДелегатЛингвистикиВопл воплощает ДелегатЛингвистики {
	...
	доступный Строка подбериМножЧислоФразы(символ[] символыФразы) {
		верни подбериЗаданнуюФормуФразы(символыФразы, Массивы.списком(Граммема.множественное));
	}

	@Подмени
	доступный Строка подбериЗаданнуюФормуФразы(символ[] символыФразы, Коллекция<Граммема> целевыеГраммемы) {
		верни операцияНаФразе(символыФразы, новый ПодборФормы(новый ФормыОтНачала(целевыеГраммемы), ЧастьРечи.существительное));
	}

	@Подмени
	доступный Строка измениНаЗаданнуюФормуСлова(символ[] символыСлова, ЧастьРечи частьРечи, Коллекция<Граммема> целевыеГраммемы) {
		верни операцияНаФразе(символыСлова, новый ПодборФормы(новый ФормыОтНачала(целевыеГраммемы), частьРечи));
	}

	@Подмени
	доступный Строка подбериНачальнуюФормуФразы(символ[] символыФразы, Коллекция<Граммема> исходныеГраммемы) {
		верни операцияНаФразе(символыФразы, новый ПодборНачальнойФормы(новый ФормыКНачалу(исходныеГраммемы), ЧастьРечи.существительное));
	}
